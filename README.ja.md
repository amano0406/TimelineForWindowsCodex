...
- `include_tool_outputs`: 互換項目。通常の `timeline.json` には tool-output log を入れません
...
Docker Compose は、project service container である `timeline-for-windows-codex-worker-1` を1つ維持し、`cli.ps1` は `docker compose exec` でその中に入って CLI を実行します。CLI 実行のたびに `worker-run-*` の一時 container を作らない方針です。ブラウザ UI はありません。
