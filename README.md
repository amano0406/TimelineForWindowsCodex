# TimelineForWindowsCodex

## Current Role

`TimelineForWindowsCodex` converts local Windows Codex Desktop history into per-thread Timeline-ready JSON artifacts. It preserves the user / assistant / system message chain so another LLM or the downstream Timeline product can consume it.

Timeline uses the worker-hosted local API for product API actions:

```text
GET http://localhost:<runtime.apiPort>/health
POST http://localhost:<runtime.apiPort>/items/refresh
POST http://localhost:<runtime.apiPort>/items/list
POST http://localhost:<runtime.apiPort>/items/detail
POST http://localhost:<runtime.apiPort>/items/download
POST http://localhost:<runtime.apiPort>/items/remove
POST http://localhost:<runtime.apiPort>/settings/status
POST http://localhost:<runtime.apiPort>/settings/init
```

The health endpoint returns a JSON boolean: `true` or `false`. All product
API actions are handled by the resident Docker worker API. API access does not
start processing implicitly; processing starts only when an API endpoint
such as `POST /items/refresh` is called.

## Scope

The source Codex history is read-only. This product writes only to:

- the configured `outputRoot`
- optional download ZIP folders
- local Docker/app runtime data

`POST /items/remove` deletes generated master item directories only. It does not delete source Codex history or existing ZIP exports. Removed items can be regenerated by calling `POST /items/refresh`.

## Input

The worker reads fixed Codex history roots through Docker Compose read-only mounts.

- Current Codex home: `C:\Users\amano\.codex`
- Known archived Codex home: `C:\Codex\archive\migration-backup-2026-03-27\codex-home`

The main transcript source is `sessions/**/*.jsonl`. `session_index.jsonl`, archived `thread_reads`, and `state_5.sqlite` fallback metadata are used only where appropriate for discovery and metadata.

## Output

The primary output is the master artifact directory configured by `settings.json`.

Current local configuration points to:

```text
C:\apps\Timeline\data\to_text\windows-codex
```

Master output layout:

```text
<outputRoot>/
  <thread_id>/
    convert_info.json
    timeline.json
```

Download ZIP layout:

```text
README.md
items/
  <thread_id>/
    convert_info.json
    timeline.json
```

See [Outputs](docs/OUTPUTS.md) for the exact `timeline.json`, `convert_info.json`, and API response JSON structures.

## Settings

`settings.json` uses this top-level order:

```json
{
  "schemaVersion": 1,
  "runtime": {
    "instanceName": "",
    "apiPort": 19200
  },
  "outputRoot": "C:\\apps\\Timeline\\data\\to_text\\windows-codex"
}
```

- `runtime.instanceName` controls the Docker Compose project name suffix.
- `runtime.apiPort` controls the local health API port.
- `outputRoot` controls where generated master item directories are written.
- Unknown fields are preserved when settings are updated.

## Quick Start

Run from the repository root:

```powershell
cd C:\apps\TimelineForWindowsCodex
.\start.bat
Invoke-RestMethod http://localhost:19200/health
Invoke-RestMethod -Method Post -Uri http://localhost:19200/settings/status -Body '{}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/refresh -Body '{}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/download -Body '{"to":"C:\\apps\\Timeline\\data\\downloads\\windows-codex"}' -ContentType 'application/json'
```

Health check:

```powershell
Invoke-RestMethod http://localhost:19200/health
```

Stop the services:

```powershell
.\stop.bat
```

## Common API Calls

```powershell
Invoke-RestMethod -Method Post -Uri http://localhost:19200/settings/init -Body '{}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/settings/status -Body '{}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/list -Body '{"page":2,"pageSize":50}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/refresh -Body '{}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/download -Body '{"to":"C:\\apps\\Timeline\\data\\downloads\\windows-codex"}' -ContentType 'application/json'
Invoke-RestMethod -Method Post -Uri http://localhost:19200/items/remove -Body '{"itemIds":["<thread_id>"]}' -ContentType 'application/json'
```

## Testing

Run the main operational check suite:

```powershell
.\test-operational.bat
```

The operational suite uses fixture Codex homes under:

- `tests/fixtures/codex-home-min`
- `tests/fixtures/archived-root-min`

It covers API refresh/download, source-to-output fidelity, Windows launcher flow, Docker Compose refresh reuse, and `POST /items/remove` against temporary fixture output.

Additional direct checks:

```powershell
python -m unittest discover -s worker\tests -v
docker compose config --quiet
```

## Known Current Limits

- `POST /items/list` can be slow on large real Codex history because discovery still scans source history before pagination is applied.
- Binary attachment contents are not exported.
- Tool-call details, terminal output, and reasoning summaries are not exported into `timeline.json`.

## Detailed Docs

- [Outputs](docs/OUTPUTS.md): exact `timeline.json`, `convert_info.json`, API response JSON, and download ZIP contract.
- [Runtime](docs/RUNTIME.md): Docker, settings, source mount, and uninstall behavior.
- [Testing](docs/TESTING.md): validation commands and what each test checks.
